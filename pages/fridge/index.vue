<template>
	<view class="container">
		<!-- 冰箱列表 -->
		<z-paging ref="paging" class="fridge-list" :fixed="false" :custom-refresher="true" :show-empty="false"
			v-model="fridgeItems" @query="queryList">
			<!-- 头部内容 -->
			<template #top>
				<!-- 统计卡片 -->
          ...
          <view class="stat-item auto-remove-toggle" @tap="toggleAutoRemove">
			...
			<!-- 冰箱卡片 -->
			<view v-for="(item, index) in sortedFridgeItems" :key="item.id" class="fridge-card"
				:class="{ 'zero-quantity': item.quantity === 0 }" :style="{ animationDelay: index * 0.05 + 's' }">
				<!-- 过期提醒 -->
				<view class="expiry-badge warning" v-if="isExpiringSoon(item.expiryDate)">
				...
				<view class="item-image-wrap">
					<image v-if="item.dishImage && !imageErrors.has(item.id)" class="item-image" :src="item.dishImage"
						mode="aspectFill" @error="onImageError(item.id)" />
				...
				<view class="item-content">
					...
					<view class="item-meta-row">
						...
						<view class="quantity-control">
							...
							<text class="quantity-value">{{ item.quantity }}{{ item.unit || "份" }}</text>
						...
						<view class="expiry-info" @tap="showExpiryPicker(item)">
							...
							<text class="expiry-date" :class="{ 'expiring-soon': isExpiringSoon(item.expiryDate) }">
		...
		<view class="dish-modal" v-if="showAddModal">
			<view class="modal-content">
				...
				<view class="modal-search-wrap">
					...
					<input class="modal-search" v-model="searchKeyword" placeholder="搜索菜品" />
				</view>

				<!-- 分类筛选 -->
				<scroll-view class="modal-category-scroll" scroll-x :scroll-with-animation="true"
					:show-scrollbar="false" enable-flex>
					<view class="modal-category-list">
						<view class="modal-category-item" :class="{ active: !modalCategory }"
							@tap="selectModalCategory('')">
							<text class="category-text">全部</text>
						</view>
						<view v-for="cat in categories" :key="cat.id" class="modal-category-item"
				...
				<scroll-view v-else class="modal-list" scroll-y :scroll-with-animation="true" :enable-flex="true"
					:enable-avoid-clip="true" :enable-passive="true" scroll-anchoring>
					<view v-for="dish in filteredDishes" :key="dish.id" class="modal-dish-item" :class="{
              adding: addingDishId === dish.id,
              'in-fridge': isInFridge(dish.id),
            }" @tap="selectDish(dish)">
						<image v-if="dish.image" class="modal-dish-img" :src="dish.image" mode="aspectFill" />
						...
						<view class="modal-dish-info">
							...
							<view class="modal-dish-meta">
								...
								<text class="modal-dish-status in-fridge-text" v-if="isInFridge(dish.id)">
									已在冰箱
								</text>
							</view>
						</view>
						<view class="modal-dish-add-btn" v-if="!isInFridge(dish.id)">
							<view v-if="addingDishId === dish.id" class="adding-spinner"></view>
							<text v-else class="add-text">添加</text>
						</view>
					</view>

					<view v-if="filteredDishes.length === 0 && dishes.length > 0" class="modal-empty">
