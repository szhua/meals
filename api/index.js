...
function mockHandler(url, method, data) {
	...
	if (url === '/dishes' && method === 'GET') {
		...
		// 分类筛选
		if (data?.category) {
			list = list.filter(d => d.categoryId === data.category)
		}
	...
	if (url === '/dishes' && method === 'POST') {
		const category = mockData.categories.find(c => c.id === data.categoryId)
		const newDish = {
			id: Date.now(),
			...data,
	...
	if (url.startsWith('/dishes/') && method === 'PUT') {
		...
		if (index !== -1) {
			const category = mockData.categories.find(c => c.id === data.categoryId)
			mockData.dishes[index] = {
				...mockData.dishes[index],
				...data,
				categoryName: category?.name || mockData.dishes[index].categoryName
			}
...
const api = {
	...
	/**
	 * 批量获取菜品
	 * @param {number[]} ids - 菜品ID数组
	 */
	getDishesBatch(ids) {
		return request('/dishes/batch', {
			method: 'POST',
			data: {
				ids
			}
	 ...
	 * @param {number} excludeId - 排除的规划ID
	 */
	 ...
	 * @returns {Promise} 返回包含完整图片URL的结果
	 */
					...
					if (data.code === 0) {
						...
						if (data.data && data.data.url) {
							...
							if (!data.data.url.startsWith('http')) {
								data.data.url = config.baseImageUrl + data.data.url
							}
						}
						resolve(data)
					} else {
	...
	/**
	 * 设置图片基础地址
	 * @param {string} url - 新的基础地址
	 */
	setBaseImageUrl(url) {
		config.baseImageUrl = url
	}
}
